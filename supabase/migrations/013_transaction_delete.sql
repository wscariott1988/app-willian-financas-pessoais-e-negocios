-- 013_transaction_delete.sql
-- Exclusão segura via soft-delete (deleted_at). Preserva todas as FKs e auditoria.
-- Para transferências: remove ambas as pontas + vínculo atomicamente.
-- category_raw é preservado como dado original imutável.

-- 0) Colunas deleted_at ANTES das funções (dependência de ordem)
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS deleted_at timestamptz DEFAULT NULL;
ALTER TABLE transfer_links ADD COLUMN IF NOT EXISTS deleted_at timestamptz DEFAULT NULL;
CREATE INDEX IF NOT EXISTS idx_tx_not_deleted ON transactions (id) WHERE deleted_at IS NULL;

-- 0b) Expandir CHECK de transaction_audit para aceitar 'delete'
ALTER TABLE transaction_audit DROP CONSTRAINT IF EXISTS transaction_audit_action_check;
ALTER TABLE transaction_audit ADD CONSTRAINT transaction_audit_action_check
...
BEGIN
    ...
    RETURN to_jsonb(v_cur.*) || jsonb_build_object(
        ...
        'updated_at',     (SELECT updated_at FROM transactions WHERE id = p_transaction_id)
    );
END;
$$;
...
BEGIN
    ...
    IF v_tx_profile <> p_profile_id THEN
        RAISE EXCEPTION 'perfil do token nao possui a transacao %', p_transaction_id;
    END IF;
    IF (SELECT deleted_at FROM transactions WHERE id = p_transaction_id) IS NOT NULL THEN
        RAISE EXCEPTION 'transacao % ja foi excluida', p_transaction_id;
    END IF;
...
-- 5) RPC transaction_delete (soft-delete atômico)
...
BEGIN
    ...
    UPDATE transactions SET deleted_at = v_now, updated_at = v_now
     WHERE id = p_transaction_id;
    ...
    IF v_is_transfer THEN
        ...
        UPDATE transfer_links SET deleted_at = v_now
         WHERE id = v_link_id;
