...
-- VERIFICADOR FINAL do chat persistente após o 025
-- (PESSOAL-13C2C.4 / PESSOAL-13C2A.4) no Supabase de TESTE.
--
--   PARTE A — ESTRUTURA + POLICIES + ACL EXATA (catálogo): pré-condições,
--             políticas por comando e os privilégios POR PRIVILÉGIO
--             (has_table_privilege) de cada tabela/role numa única grade
--             exportável (PASS/BLOCKED). Exige que chat_messages tenha
--             SOMENTE policies SELECT/INSERT/UPDATE e NENHUMA policy de
--             DELETE individual (a chat_messages_delete_own do 023 foi
--             removida pelo 025).
...
--             via cascade da conversa própria e — contrato central — que
--             DELETE direto em chat_messages levanta EXATAMENTE 42501.
--             NÃO se aceita "DELETE 0": o teste falha se a operação concluir
--             sem erro.
--
-- Histórico do erro real no TESTE (PESSOAL-13C2A.3): o bootstrap do
-- Supabase aplica default privileges no schema public que concedem ALL ON
-- TABLES aos roles anon/authenticated e ao role de serviço; o 023 criou as
-- tabelas com grants mínimos sem revogar o herdado. O 024 revogou
-- PUBLIC/anon/authenticated e reaplicou os grants mínimos. O 025 elimina a
-- policy latente chat_messages_delete_own (inativa pela ACL, mas
-- desnecessária e redutora da defesa em profundidade). Nenhuma policy de
-- DELETE de mensagem individual deve existir.
--
...
-- chamada direta sob authenticated/anon.
--
...
-- ============================================================
-- PARTE A — VERIFICAÇÃO ESTRUTURAL, DE POLICIES E DE ACL (catálogo)
-- ============================================================
SELECT * FROM (
    ...
    SELECT 10 AS ord, 'A_indices_por_nome_estavel' AS stage,
    ...
    SELECT 11 AS ord, 'A_cascata_mensagens' AS stage,
    ...
    SELECT 12 AS ord, 'A_acl_authenticated_conversations' AS stage,
    ...
    SELECT 13 AS ord, 'A_acl_authenticated_messages' AS stage,
    ...
    SELECT 14 AS ord, 'A_acl_anon_nenhum_privilegio' AS stage,
    ...
    SELECT 15 AS ord, 'A_acl_public_default_revogado' AS stage,
...
BEGIN
    ...
    IF to_regclass('public.chat_conversations') IS NULL
       ...
       OR NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
        RAISE NOTICE 'VERIFY 025 (comportamento): BLOCKED - pre-condicao estrutural ausente (nenhum DML executado)';
        RETURN;
    END IF;
    ...
    IF v_p1 IS NULL OR v_p2 IS NULL THEN
        RAISE NOTICE 'VERIFY 025 (comportamento): BLOCKED - menos de dois perfis para isolar (nenhum DML executado)';
        RETURN;
    END IF;
    ...
    IF v_n <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A nao enxerga a propria conversa'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_conversations WHERE id = v_conv_b;
    IF v_n <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A enxerga conversa do perfil B'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_messages WHERE conversation_id = v_conv_a;
    IF v_n <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A nao ve mensagens da propria conversa'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_messages WHERE conversation_id = v_conv_b;
    IF v_n <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A ve mensagens da conversa do perfil B'; END IF;
    ...
    IF v_n <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A nao atualiza a propria conversa'; END IF;
    UPDATE public.chat_conversations SET title = 'hack' WHERE id = v_conv_b;
    GET DIAGNOSTICS v_linhas = ROW_COUNT;
    IF v_linhas <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A alterou conversa do perfil B'; END IF;
    RAISE NOTICE 'VERIFY 025 OK: A ve e edita so a propria conversa (SELECT/UPDATE)';
    ...
    IF v_n <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: INSERT+UPDATE de mensagem (idempotencia) quebrado'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_messages WHERE conversation_id = v_conv_a;
    IF v_n <> 2 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: contagem de mensagens da conversa A divergente'; END IF;
    RAISE NOTICE 'VERIFY 025 OK: INSERT + UPDATE de mensagem (upsert/idempotencia) funcionando';
    ...
    IF NOT v_blocked THEN
        RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A inseriu mensagem na conversa do perfil B';
    END IF;

    -- CONTRATO CENTRAL: DELETE direto de mensagem (mesmo da PRÓPRIA conversa,
    -- com linha visível) deve falhar com EXATAMENTE 42501 — sem grant DELETE
    -- em chat_messages E sem policy de DELETE individual. NÃO se aceita
    -- "DELETE 0": concluir sem erro quer dizer vazamento de ACL/policy.
    v_blocked := false;
    BEGIN
    ...
    END;
    IF NOT v_blocked THEN
        RAISE EXCEPTION 'VERIFY 025 FAIL: authenticated concluiu DELETE individual de mensagem';
    END IF;
    RAISE NOTICE 'VERIFY 025 OK: DELETE direto de mensagem negado exatamente com 42501';

    -- A: DELETE direto também na conversa do perfil B (sem linhas visíveis;
    -- mesmo assim o privilégio DELETE inexistente leva a 42501).
    v_blocked := false;
    BEGIN
    ...
    END;
    IF NOT v_blocked THEN
        RAISE EXCEPTION 'VERIFY 025 FAIL: authenticated (A) concluiu DELETE de mensagem da conversa B';
    END IF;
    ...
    GET DIAGNOSTICS v_linhas = ROW_COUNT;
    IF v_linhas <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil A apagou conversa do perfil B'; END IF;
    ...
    SELECT count(*) INTO v_n FROM public.chat_conversations WHERE id = v_conv_b;
    IF v_n <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil B nao enxerga a propria conversa'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_conversations WHERE id = v_conv_a;
    IF v_n <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil B enxerga conversa do perfil A'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_messages WHERE conversation_id = v_conv_b;
    IF v_n <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil B nao ve mensagens da propria conversa'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_messages WHERE conversation_id = v_conv_a;
    IF v_n <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil B ve mensagens da conversa do perfil A'; END IF;
    ...
    IF v_n <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil B nao atualiza a propria conversa'; END IF;
    UPDATE public.chat_conversations SET title = 'hack' WHERE id = v_conv_a;
    GET DIAGNOSTICS v_linhas = ROW_COUNT;
    IF v_linhas <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil B alterou conversa do perfil A'; END IF;
    ...
    END;
    IF NOT v_blocked THEN
        RAISE EXCEPTION 'VERIFY 025 FAIL: authenticated (B) concluiu DELETE individual de mensagem';
    END IF;
    ...
    -- e todas as mensagens por cascade.
    DELETE FROM public.chat_conversations WHERE id = v_conv_b;
    GET DIAGNOSTICS v_linhas = ROW_COUNT;
    IF v_linhas <> 1 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: perfil B nao apagou a propria conversa'; END IF;
    ...
    SELECT count(*) INTO v_n FROM public.chat_messages WHERE conversation_id = v_conv_b;
    IF v_n <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: cascade nao removeu mensagens da conversa B'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_messages WHERE conversation_id = v_conv_a;
    IF v_n <> 2 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: cascade afetou mensagens da conversa A'; END IF;
    SELECT count(*) INTO v_n FROM public.chat_conversations WHERE id = v_conv_b;
    IF v_n <> 0 THEN RAISE EXCEPTION 'VERIFY 025 FAIL: conversa B ainda existe apos DELETE'; END IF;
    RAISE NOTICE 'VERIFY 025 OK: A e B isolados; DELETE da propria conversa remove mensagens por cascade';
    ...
    END;
    IF NOT v_blocked THEN
        RAISE EXCEPTION 'VERIFY 025 FAIL: chave unica de idempotencia nao bloqueou duplicata';
    END IF;
    ...
    END;
    IF NOT v_blocked THEN
        RAISE EXCEPTION 'VERIFY 025 FAIL: anon leu chat_conversations';
    END IF;
    RAISE NOTICE 'VERIFY 025 OK: anon sem acesso a chat_conversations (42501)';

    -- Reset limpo de role e claims antes do ROLLBACK.
    RESET ROLE;
    PERFORM set_config('request.jwt.claims', '', true);
    RAISE NOTICE 'VERIFY 025 OK: comportamento ACL/RLS validado com policy de DELETE removida (A/B/anon) — ROLLBACK a seguir';
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM public.chat_conversations WHERE id IN (
        ...
        RAISE WARNING 'VERIFY 025: residuo de dados sinteticos encontrado (nada deveria persistir)';
    ELSE
        RAISE NOTICE 'VERIFY 025 OK: ROLLBACK confirmado - nenhum dado persistido';
    END IF;
END $$;