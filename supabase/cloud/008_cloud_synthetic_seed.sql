-- 008_cloud_synthetic_seed.sql — Seed SINTETICO para Supabase Cloud (APLICAR NO SQL EDITOR)
-- Somente dados fictícios de demonstração. Nenhum dado financeiro real é copiado.
-- Os profiles sao localizados DINAMICAMENTE em public.profiles por code
-- ('personal' e 'business'); nenhum UUID de perfil fixo e usado.
-- Validacao: se algum dos dois perfis estiver ausente, o seed falha com erro
-- explicito e NADA e inserido (bloco transacional unico).
-- Idempotente: pode ser reexecutado sem duplicar registros.
-- REQUISITO: execute apos o SETUP_SUPABASE_CLOUD.sql e apos criar os usuarios no
-- Auth (o trigger handle_new_user cria os profiles no cadastro).
