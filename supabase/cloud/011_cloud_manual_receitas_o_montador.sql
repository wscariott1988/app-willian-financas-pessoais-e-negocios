-- ============================================================
-- 011_cloud_manual_receitas_o_montador.sql  (Cloud 011 <- local 010)
-- Correção de DADOS específica do dataset real presente no Cloud
-- (100% comprovado pelo verificador vivo: 100 IDs, soma 326876.52,
-- 64/19/17, 19 filas RP-MAL-01 open). NÃO é migration estrutural
-- genérica e NÃO depende de seed sintético.
-- Decisão manual do proprietário (CSV o-montador-receitas-review,
-- manual_decision = RECEITA_PESSOAL em 100/100).
...
BEGIN
    ...
    IF (SELECT count(DISTINCT x) FROM unnest(v_ids) x) <> 100 THEN
        RAISE EXCEPTION 'CLOUD011: lista de UUIDs nao tem 100 distintos';
    END IF;

    -- 2) todos existem
    IF EXISTS (SELECT 1 FROM unnest(v_ids) x
               LEFT JOIN transactions t ON t.id = x WHERE t.id IS NULL) THEN
        RAISE EXCEPTION 'CLOUD011: algum UUID nao existe em transactions';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM transactions t JOIN unnest(v_ids) x ON x = t.id
        ...
        RAISE EXCEPTION 'CLOUD011: transacao fora do estado esperado (perfil/kind/categoria)';
    END IF;

    -- 4) soma exata 326876.52
    IF (SELECT sum(t.amount) FROM transactions t JOIN unnest(v_ids) x ON x = t.id) <> 326876.52 THEN
        RAISE EXCEPTION 'CLOUD011: soma das 100 diverge de 326876.52';
    END IF;
    ...
    OR (SELECT count(*) FILTER (WHERE t.status = 'scheduled') FROM transactions t JOIN unnest(v_ids) x ON x = t.id) <> 17 THEN
        RAISE EXCEPTION 'CLOUD011: distribuicao de status diverge de 64/19/17';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM categories
        ...
        RAISE EXCEPTION 'CLOUD011: categoria destino fora do estado esperado';
    END IF;
    IF (SELECT count(*) FROM transactions WHERE category_id = v_dest) <> 0 THEN
        RAISE EXCEPTION 'CLOUD011: categoria destino ja possui transacoes';
    END IF;
    ...
    IF (SELECT count(*) FROM reclassification_queue q
        ...
        RAISE EXCEPTION 'CLOUD011: esperadas exatamente 19 filas open RP-MAL-01';
    END IF;
END $$;
...
-- auditoria das 100 decisoes manuais
...
SELECT gen_random_uuid(), t.id,
       ...
       'cloud_011:manual_decision:RECEITA_PESSOAL'
  FROM transactions t
 WHERE t.id IN (
    '200d4021-7608-584f-95f4-80818ffa3d79',
    '6fc23233-d72e-5a32-acce-9c879953bdc3',
...
BEGIN
    ...
    IF n <> 100 THEN
        RAISE EXCEPTION 'CLOUD011: validacao posterior falhou (nao sao 100 na categoria destino)';
    END IF;
    IF (SELECT sum(t.amount) FROM transactions t JOIN unnest(v_ids) x ON x = t.id) <> 326876.52 THEN
        RAISE EXCEPTION 'CLOUD011: soma alterada na validacao posterior';
    END IF;
    IF EXISTS (SELECT 1 FROM transactions t
               JOIN categories c ON c.id = t.category_id
               WHERE t.profile_id <> c.profile_id) THEN
        RAISE EXCEPTION 'CLOUD011: cruzamento transacao x categoria por perfil apos a migracao';
    END IF;
    IF EXISTS (SELECT 1 FROM reclassification_queue q
               JOIN unnest(v_ids) x ON x = q.transaction_id
               WHERE q.status = 'open' AND q.reason = 'RP-MAL-01') THEN
        RAISE EXCEPTION 'CLOUD011: restam filas RP-MAL-01 abertas';
    END IF;
    SELECT count(*) INTO n FROM category_assignment_audit
     WHERE reason = 'cloud_011:manual_decision:RECEITA_PESSOAL' AND assigned_by IS NULL;
    IF n <> 100 THEN
        RAISE EXCEPTION 'CLOUD011: esperadas 100 auditorias cloud_011';
    END IF;
END $$;
