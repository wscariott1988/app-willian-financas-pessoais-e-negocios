...
describe('Auditoria: migração Início → Configurações', () => {
  it('45) Início não renderiza AuditLogs nem "Logs Recentes de Auditoria"', () => {
    const src = readSource('components/Dashboard.tsx');
    expect(src).not.toContain('AuditLogs');
    expect(src).not.toContain('Logs Recentes de Auditoria');
    const html = renderToString(createElement(Dashboard, { profileId: shellProps.profileId, period }));
    expect(html).not.toContain('settings-audit');
    expect(html).not.toContain('Histórico de alterações');
  });

  it('46) Configurações renderiza SettingsView com "Histórico de alterações"', () => {
    const html = renderToString(
      createElement(AppShell, { ...shellProps, initialView: 'configuracoes' }),
    );
    expect(html).toContain('Histórico de alterações');
    expect(html).toContain('settings-audit');
  });

  it('47) Configurações exibe descrição curta orientativa', () => {
    const html = renderToString(
      createElement(AppShell, { ...shellProps, initialView: 'configuracoes' }),
    );
    expect(html).toContain('Consulte as alterações realizadas nas suas transações e categorias.');
  });

  it('48) AuditLogs usa profileId do perfil ativo na consulta', () => {
    const src = readSource('components/AuditLogs.tsx');
    expect(src).toContain('profileId');
    expect(src).toMatch(/interface AuditLogsProps/);
    expect(src).toContain('profileId: string');
  });

  it('49) AuditLogs exibe estados amigáveis: carregando, vazio, erro', () => {
    const src = readSource('components/AuditLogs.tsx');
    expect(src).toContain('Carregando...');
    expect(src).toContain('Nenhuma alteração registrada.');
    expect(src).toContain('Não foi possível carregar o histórico de alterações');
  });

  it('50) AuditLogs não expõe UUIDs, nomes de tabelas ou RPCs no JSX renderizado', () => {
    const src = readSource('components/AuditLogs.tsx');
    const jsxSection = src.slice(src.indexOf('return ('));
    expect(jsxSection).not.toContain('transaction_id');
    expect(jsxSection).not.toContain('queue_item_id');
    expect(jsxSection).not.toContain('category_assignment_audit');
    expect(jsxSection).not.toContain('audit_select_own');
  });

  it('51) AuditLogs é somente leitura — não renderiza botões de edição/exclusão', () => {
    const src = readSource('components/AuditLogs.tsx');
    expect(src).not.toContain('onEditTransaction');
    expect(src).not.toContain('onDeleteTransaction');
    expect(src).not.toContain('Pencil');
    expect(src).not.toContain('Trash2');
  });

  it('52) AuditLogs possui "Carregar mais" com paginação (PAGE_SIZE = 10)', () => {
    const src = readSource('components/AuditLogs.tsx');
    expect(src).toContain('PAGE_SIZE = 10');
    expect(src).toContain('Carregar mais');
    expect(src).toContain('loadingMore');
    expect(src).toContain('hasMore');
    expect(src).toContain('handleLoadMore');
  });

  it('53) AuditLogs ordena por created_at decrescente (mais recentes primeiro)', () => {
    const src = readSource('components/AuditLogs.tsx');
    expect(src).toContain("order('created_at', { ascending: false })");
  });

  it('54) CSS contém estilos para settings-audit-*', () => {
    expect(css).toContain('settings-view');
    expect(css).toContain('settings-audit');
    expect(css).toContain('settings-audit-header');
    expect(css).toContain('settings-audit-title');
    expect(css).toContain('settings-audit-desc');
    expect(css).toContain('settings-audit-list');
    expect(css).toContain('settings-audit-entry');
    expect(css).toContain('settings-audit-state');
    expect(css).toContain('settings-audit-error');
    expect(css).toContain('settings-audit-load-more');
  });

  it('55) AuditLogs não renderiza termos técnicos como UUIDs, tabelas ou RPCs no JSX', () => {
    const src = readSource('components/AuditLogs.tsx');
    const jsxSection = src.slice(src.indexOf('return ('));
    expect(jsxSection).not.toContain('category_assignment_audit');
    expect(jsxSection).not.toContain('audit_select_own');
    expect(jsxSection).not.toContain('transaction_id');
    expect(jsxSection).not.toContain('queue_item_id');
    expect(jsxSection).not.toContain('Canonical Path');
    expect(jsxSection).not.toContain('canonical_path');
  });
  ...
  it('57) Configurações respeita isolamento Pessoal × Negócio via profileId', () => {
    const personalHtml = renderToString(
      createElement(AppShell, { ...shellProps, initialView: 'configuracoes', profileCode: 'personal' }),
    );
    expect(personalHtml).toContain('Histórico de alterações');
    expect(personalHtml).toContain('Configurações');
  });
});
